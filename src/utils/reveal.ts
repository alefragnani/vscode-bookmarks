...
function resolveNavigationColumn(line: number, storedColumn: number): number {
    const columnMode = workspace
        .getConfiguration("bookmarks")
        .get<string>("navigation.columnMode", "stored");
...
export function revealLine(line: number, directJump?: boolean) {
    ...
    window.activeTextEditor.revealRange(
        newSe,
        getRevealLocationConfig(directJump),
    );
}
...
export function revealPosition(line: number, column: number): void {
    if (isNaN(column)) {
        ...
        const revealType = getRevealLocationConfig(
            line === window.activeTextEditor.selection.active.line,
        );
        const newPosition = new Selection(
            line,
            resolvedColumn,
            line,
            resolvedColumn,
...
export async function previewPositionInDocument(
    point: Bookmark,
    uri: Uri,
): Promise<void> {
...
export async function revealPositionInDocument(
    point: Bookmark,
    uri: Uri,
): Promise<void> {
    const textDocument = await workspace.openTextDocument(uri);
